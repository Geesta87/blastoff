...
import type { EmailTemplate, Tag, Segment } from '@/lib/types'
...
export default function NewEmailCampaignPage() {
  ...
  // Set defaults from workspace when loaded
  useEffect(() => {
    if (workspace) {
      setForm((prev) => ({
        ...prev,
        from_name: prev.from_name || workspace.sendgrid_from_name || 'Blastoff',
        from_email: prev.from_email || workspace.sendgrid_from_email || '',
      }))
    }
  }, [workspace])
  ...
  const fetchRecipientCount = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/campaigns/email/count', {
        method: 'POST',
  ...
  function handleTemplateSelect(templateId: string) {
    if (templateId === 'blank') {
      updateForm({ template_id: '', subject: '', html_body: '', preview_text: '' })
      return
    }
    ...
    if (template) {
      updateForm({
        ...
        html_body: template.html_body || form.html_body,
        preview_text: template.preview_text || form.preview_text,
      })
    }
  }

  function insertSubjectMergeTag(tag: string) {
    ...
    updateForm({ subject: before + tag + after })
  ...
  async function handleSendTest() {
    ...
    setTestResult(null)
    try {
      const res = await fetch('/api/campaigns/email/test', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          to_email: testEmail,
  ...
  async function handleConfirmSend() {
    ...
    try {
      ...
      const createRes = await fetch('/api/campaigns/email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (form.schedule_type === 'now') {
        const sendRes = await fetch(`/api/campaigns/email/${campaign.id}/send`, {
          method: 'POST',
        })
        if (!sendRes.ok) {
  ...
  function canProceed(): boolean {
    switch (currentStep) {
      ...
      case 2:
        return form.subject.trim().length > 0 && form.html_body.trim().length > 0
      case 3:
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        <h1 className="text-3xl font-bold">New Email Campaign</h1>
      ...
      {currentStep === 2 && (
        <Card>
          ...
          <CardContent className="space-y-4">
              ...
              <div className="flex flex-wrap gap-1">
                {MERGE_TAGS.map((item) => (
                  <Button
                    ...
                    onClick={() => insertSubjectMergeTag(item.tag)}
                  >
                    {item.label}
                  </Button>
                ))}
            ...
            <EmailEditor
              subject={form.subject}
              onSubjectChange={(val) => updateForm({ subject: val })}
              previewText={form.preview_text}
              onPreviewTextChange={(val) => updateForm({ preview_text: val })}
              htmlBody={form.html_body}
              onHtmlBodyChange={(val) => updateForm({ html_body: val })}
              showSubject={false}
            />
              ...
              <div className="flex items-center space-x-3 rounded-md border p-4">
                ...
                <Label htmlFor="recipient-all" className="cursor-pointer flex-1">
                  ...
                  <div className="text-sm text-muted-foreground">
                    Send to every active contact with a valid email address
                  </div>
                </Label>
              </div>
              ...
              <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                ...
                {form.reply_to && (
                  <div>
                    ...
                    <dd className="font-medium">{form.reply_to}</dd>
                  </div>
                )}
              </dl>
            </CardContent>
              ...
              <div className="rounded-md border bg-muted/30 overflow-hidden">
                ...
                <div className="p-4 flex justify-center">
                  <iframe
                    ...
                    className="w-full max-w-[600px] min-h-[300px] rounded border bg-white"
      ...
      <Dialog open={showTestDialog} onOpenChange={setShowTestDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSendTest} disabled={!testEmail || testSending}>
